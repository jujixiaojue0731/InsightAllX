...
Standard ACP image, `resource_link`, and URI-backed `resource` content blocks are preferred and render directly. insightAll ACP currently projects assistant text and thought content but can omit assistant media, while Gateway processing removes `MEDIA:` directives from the visible live reply. insightAllX handles those gaps through two bounded in-memory compatibility exceptions. Neither revives the legacy Chat renderer nor represents synthetic data as a native ACP event.
