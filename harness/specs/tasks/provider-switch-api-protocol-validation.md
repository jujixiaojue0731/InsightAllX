...
title: Reject invalid insightAll api protocol writes and self-heal stale entries
scenario: gateway-backend-communication
taskType: runtime-bridge
intent: Prevent insightAllX from writing models.providers.*.api values outside the insightAll allowlist into openclaw.json, and opportunistically prune pre-existing invalid entries when the user switches default provider so a clean switch can rescue the Gateway from Invalid-config restart loops.
touchedAreas:
...
expectedUserBehavior:
  ...
  - Attempting to save a provider whose api protocol is not in the insightAll allowlist surfaces an actionable error in the UI without polluting openclaw.json or triggering a Gateway restart.
...
acceptance:
  ...
  - setinsightAllDefaultModel, setinsightAllDefaultModelWithOverride, syncProviderConfigToinsightAll, and upsertinsightAllProviderEntry all invoke assertValidApiProtocol before any write to openclaw.json.
...
`providerConfig.api` to the literal string `'openrouter'`, which is not in insightAll's allowed
...
When the user selected OpenRouter as their default provider, insightAllX wrote that invalid value into
...
- Add a single source of truth for the allowed api protocols and a runtime guard at every write
  site (`setinsightAllDefaultModel`, `setinsightAllDefaultModelWithOverride`,
  `syncProviderConfigToinsightAll`, plus defense-in-depth inside `upsertinsightAllProviderEntry`).
