...
touchedAreas:
  ...
  - resources/openclaw-plugins/insightall-openai-image/index.mjs
  - resources/openclaw-plugins/insightall-openai-image/openclaw.plugin.json
  - resources/openclaw-plugins/insightall-openai-image/package.json
...
expectedUserBehavior:
  ...
  - Saving the OpenAI-compatible image endpoint writes a InsightAll-owned provider (`insightall-openai-image`) and auth profile, enables `request.allowPrivateNetwork` for trusted custom endpoints, and leaves `models.providers.openai` untouched so chat continues to use the regular OpenAI provider.
...
`~/.openclaw/agents/{id}/agent/auth-profiles.json`. InsightAll's OpenAI-compatible
image endpoint uses a separate `insightall-openai-image` provider/plugin so image
base URL and API key can differ from the normal `openai` chat provider.

InsightAll syncs chat defaults on provider switch, but image generation is configured independently from its developer-only Image Generation page and is never auto-synced from the default chat provider.
