...
expectedUserBehavior:
  ...
  - insightAll's trajectory artefacts for the same session id are removed too — both the local <id>.trajectory.jsonl flight recorder and the <id>.trajectory-path.json pointer sidecar.
  ...
  - The session entry is removed from sessions.json so insightAll sessions.list stops returning it.
...
acceptance:
  ...
  - The sweep also unlinks <id>.trajectory.jsonl and <id>.trajectory-path.json sidecars produced by insightAll's runtime trajectory writer.
...
Conversation deletion in insightAllX runs entirely on the Main process — the
insightAll Gateway does not expose a `sessions.delete` RPC. Historically the
...
share the same disk contract via `electron/utils/session-files.ts`, which centralises:
  ...
  - the trajectory pointer-follow that handles insightAll's
