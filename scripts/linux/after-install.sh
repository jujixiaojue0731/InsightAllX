...
# Post-installation script for insightAllX on Linux
...
# Create symbolic link for insightAllX app binary
if [ -x /opt/insightAllX/insightallx ]; then
    ln -sf /opt/insightAllX/insightallx /usr/local/bin/insightallx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/insightAllX/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/insightAllX/chrome-sandbox' || true
else
    chmod 0755 '/opt/insightAllX/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/insightAllX/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/insightallx'
...
echo "insightAllX has been installed successfully."
