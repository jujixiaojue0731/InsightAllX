...
# Post-installation script for InsightAll on Linux
...
# Create symbolic link for InsightAll app binary
if [ -x /opt/InsightAll/insightall ]; then
    ln -sf /opt/InsightAll/insightall /usr/local/bin/insightall 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/InsightAll/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/InsightAll/chrome-sandbox' || true
else
    chmod 0755 '/opt/InsightAll/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/InsightAll/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/insightall'
...
echo "InsightAll has been installed successfully."
