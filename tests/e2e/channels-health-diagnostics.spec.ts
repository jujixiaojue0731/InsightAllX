...
test.describe('Channels health diagnostics', () => {
  ...
  test('shows degraded banner, restarts gateway, and copies diagnostics', async ({ electronApp, page }) => {
    await electronApp.evaluate(({ ipcMain }) => {
      ...
      (globalThis as any).__insightallxE2eChannelHealth = state;
      ...
      ipcMain.handle('host:invoke', async (event, request: { id?: string; module?: string; action?: string }) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const current = (globalThis as any).__insightallxE2eChannelHealth as typeof state;
        ...
        if (request?.module === 'diagnostics' && request.action === 'gatewaySnapshot') {
          ...
          return respond(request.id, {
            ...
            channels: [],
            insightallxLogTail: 'insightallx-log',
    ...
    const result = await electronApp.evaluate(() => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const state = (globalThis as any).__insightallxE2eChannelHealth as { restartCount: number; diagnosticsCount: number };
      return {
