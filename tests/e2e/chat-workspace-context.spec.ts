...
const SESSION_WORKSPACE = '/Users/e2e/workspace/InsightAll';
const SESSION_WORKSPACE_LABEL = 'InsightAll';
...
test.describe('InsightAll chat workspace context', () => {
  test('bound session shows read-only workspace and workspace tree uses the same cwd', async ({ launchElectronApp }) => {
    ...
    try {
      ...
      await expect(workspaceSelector).toContainText('InsightAll');
