  ...
  useTranslation: () => ({
    t: (key: string, options?: Record<string, unknown>) => {
      ...
      const labels: Record<string, string> = {
        ...
        'fileCard.openInBuiltInBrowser': 'Open in InsightAll Preview',
...
describe('ACP chat timeline components', () => {
  ...
  it('repairs only the active final assistant Markdown part and scopes animation markers to it', async () => {
    ...
    const activeStreamdownRoot = activeSegment?.querySelectorAll('.insightall-streamdown')[1];
    ...
    const settledStreamdownRoot = activeSegment?.querySelectorAll('.insightall-streamdown')[1];
  ...
  it('opens HTML file activity in the built-in browser from the first Open with item', async () => {
    ...
    expect(items[0]).toHaveTextContent('Open in InsightAll Preview');
  ...
  it('renders a user image attachment as a thumbnail with a filename hover overlay', async () => {
    const ref = {
      sessionKey: 'agent:main:s1', generation: 1, uri: '/tmp/insightall-staging/photo.png', stagingId: 'stage-photo',
    };
  ...
  it('shows a user file path after its name without MIME or size and keeps preview routing', () => {
    const ref = {
      sessionKey: 'agent:main:s1', generation: 1, uri: '/tmp/insightall-staging/notes.txt', stagingId: 'stage-notes',
    };
    const state = snapshot({
      itemOrder: ['msg-u:0'],
      itemsById: {
  ...
  it('opens an unsupported user file through the scoped system-open route', async () => {
    const ref = {
      sessionKey: 'agent:main:s1', generation: 1, uri: '/tmp/insightall-staging/archive.zip', stagingId: 'stage-zip',
    };
    const state = snapshot({
      itemOrder: ['msg-u:0'],
      itemsById: {
