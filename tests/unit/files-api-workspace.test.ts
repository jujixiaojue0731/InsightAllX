...
vi.mock('electron', () => ({
  app: { getPath: vi.fn(() => '/tmp/insightall-user-data') },
...
describe('workspace-scoped files api', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = await mkdtemp(join(tmpdir(), 'insightall-files-workspace-'));
    mocks.home = testDir;
  ...
  it('registers staged file and directory ids with Main-owned source storage', async () => {
    ...
    expect(result.stagedPath).toContain(join('media', 'outbound', 'insightall-staging'));
    expect(pathResult.stagedPath).toContain(join('media', 'outbound', 'insightall-staging'));
