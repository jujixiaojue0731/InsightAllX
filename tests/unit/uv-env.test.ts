...
beforeEach(() => {
  tempHome = mkdtempSync(path.join(tmpdir(), 'insightallx-uv-env-'));
  vi.resetModules();
  ...
  vi.doMock('electron', () => ({
    app: {
      ...
      getPath: vi.fn().mockReturnValue(path.join(tempHome, '.insightallx')),
...
describe('uv mirror environment', () => {
  it('writes a insightAllX-managed uv.toml under ~/.openclaw and exposes it via UV_CONFIG_FILE', async () => {
    const { getUvMirrorEnv } = await import('@electron/utils/uv-env');

    const env = await getUvMirrorEnv();

    const expectedPath = path.join(tempHome, '.openclaw', 'insightallx', 'uv.toml');
