...
- 高度なプロキシ項目が空の場合、insightAllXは **プロキシサーバー** にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、Gatewayが自動的に再起動します。
- Telegramが有効な場合、insightAllXはプロキシをinsightAllのTelegramチャネル設定にも同期します。
- insightAllXのプロキシが無効な状態で通常のGateway再起動が行われても、既存のTelegramチャネルプロキシは保持されます。
