...
- If advanced proxy fields are left empty, insightAllX falls back to **Proxy Server**.
- Saving proxy settings reapplies Electron networking immediately and restarts the Gateway automatically.
- When Telegram is enabled, insightAllX also syncs the proxy to insightAll's Telegram channel configuration.
- When the insightAllX proxy is disabled, a normal Gateway restart preserves an existing Telegram channel proxy.
