...
import { getInsightAllProviderStore } from '../services/providers/store-instance';
import {
...
export async function storeApiKey(providerId: string, apiKey: string): Promise<boolean> {
  try {
    ...
    const s = await getInsightAllProviderStore();
 ...
 * Also synchronizes InsightAll local provider list with insightAll's actual config.
 */
...
> {
  ...
  for (const provider of providers) {
    ...
    // we should remove it from InsightAll UI to stay consistent.
