...
import { getActiveinsightAllProviders } from './openclaw-auth';
import {
...
import { getinsightAllXProviderStore } from '../services/providers/store-instance';
import {
  deleteProviderSecret,
  getProviderSecret,
  setProviderSecret,
} from '../services/secrets/secret-store';
import { getinsightAllProviderKeyForType } from './provider-keys';
...
export async function storeApiKey(providerId: string, apiKey: string): Promise<boolean> {
  try {
    ...
    const s = await getinsightAllXProviderStore();
 ...
 * Also synchronizes insightAllX local provider list with insightAll's actual config.
 */
...
> {
  ...
  const activeinsightAllProviders = await getActiveinsightAllProviders();
  ...
  const configMissing = activeinsightAllProviders.size === 0;

  for (const provider of providers) {
    // Sync check: If it's a custom/OAuth provider and it no longer exists in insightAll config
    // (e.g. wiped by Gateway due to missing plugin, or manually deleted by user)
    // we should remove it from insightAllX UI to stay consistent.
    const isBuiltin = (BUILTIN_PROVIDER_TYPES as readonly string[]).includes(provider.type);
    // For custom/ollama providers, the insightAll config key is derived as
    ...
    // This must match getinsightAllProviderKey() in ipc-handlers.ts exactly.
    const openClawKey = getinsightAllProviderKeyForType(provider.type, provider.id);
    const isActive = activeinsightAllProviders.has(provider.type) || activeinsightAllProviders.has(provider.id) || activeinsightAllProviders.has(openClawKey);
    if (configMissing || (!isBuiltin && !isActive)) {
      console.log(`[Sync] Provider ${provider.id} (${provider.type}) missing from insightAll, hiding from UI`);
