...
const INSTALLED_PLUGIN_INDEX_WARNING = 'DO NOT EDIT. This file is generated by insightAll from plugin manifests, install records, and config policy. Use `openclaw plugins registry --refresh`, `openclaw plugins install/update/uninstall`, or `openclaw plugins enable/disable` instead.';
...
function resolveinsightAllStateDir(): string {
  return process.env.OPENCLAW_STATE_DIR?.trim() || join(homedir(), '.openclaw');
}

function resolveinsightAllStateSqlitePath(): string {
  // insightAll 2026.7.1 moved the shared state database under state/.
  // Writing the legacy root-level database leaves Gateway migrations reading
  // stale plugin records from the canonical database.
  return join(resolveinsightAllStateDir(), 'state', 'openclaw.sqlite');
}
...
export function upsertPluginInstallRecordsIntoSqlite(
  ...
  const sqlitePath = resolveinsightAllStateSqlitePath();
  mkdirSync(join(resolveinsightAllStateDir(), 'state'), { recursive: true });
...
export function removePluginInstallRecordsFromSqlite(pluginIds: string[]): boolean {
  ...
  const stateDir = resolveinsightAllStateDir();
  const sqlitePaths = [
    resolveinsightAllStateSqlitePath(),
...
export function ensureinsightAllStateDirExists(): void {
  const stateDir = resolveinsightAllStateDir();
