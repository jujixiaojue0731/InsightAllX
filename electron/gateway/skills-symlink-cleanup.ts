/**
 * Pre-launch cleanup for stray skill symlinks under insightAll skill roots.
 *
...
import { getinsightAllConfigDir, getinsightAllResolvedDir, getinsightAllSkillsDir } from '../utils/paths';
...
export interface PluginRuntimeDepsCleanupOptions {
  ...
  /** Override for the current bundled insightAll package dir (mainly for tests). */
  currentinsightAllDir?: string;
}

function defaultSkillsDir(): string {
  return getinsightAllSkillsDir();
}
...
function defaultWorkspaceSkillsDir(): string {
  return path.join(getinsightAllConfigDir(), 'workspace', 'skills');
}

function defaultWorkspaceAgentsDir(): string {
  return path.join(getinsightAllConfigDir(), 'workspace', '.agents', 'skills');
}

function defaultPluginRuntimeDepsDir(): string {
  return path.join(getinsightAllConfigDir(), 'plugin-runtime-deps');
}
...
function looksLikeinsightAllPackagePath(candidate: string): boolean {
  const normalized = candidate.replace(/\\/g, '/');
  return /\/node_modules(?:\/\.pnpm\/[^/]+\/node_modules)?\/openclaw(?:\/|$)/.test(normalized);
}

function resolveCurrentinsightAllRoots(currentinsightAllDir: string): string[] {
  const roots = new Set<string>([path.resolve(currentinsightAllDir)]);
  try {
    roots.add(realpathSync(currentinsightAllDir));
  } catch {
    // fall through
  }
...
/**
 * Remove stale insightAll plugin runtime dependency cache roots.
 *
 * insightAll can materialize `~/.openclaw/plugin-runtime-deps/openclaw-*` as a
 ...
 * are removed, and only when a symlink inside points at an insightAll package
 * path outside the current bundled package.  The cache is regenerated by
 * insightAll on demand.
 */
export function cleanupStalePluginRuntimeDeps(
  ...
  const currentRoots = resolveCurrentinsightAllRoots(opts.currentinsightAllDir ?? getinsightAllResolvedDir());
  const result: CleanupResult = { removed: [], examined: 0 };
  ...
  for (const entry of entries) {
    ...
    const scan = scanRuntimeDepsRootForStaleinsightAllSymlink(cacheRoot, currentRoots);
  ...
  if (result.removed.length > 0) {
    logger.info(
      `[plugin-runtime-deps-cleanup] Removed ${result.removed.length} stale insightAll runtime cache root(s): ` +
        result.removed.join(', '),
    );
  }
...
function scanRuntimeDepsRootForStaleinsightAllSymlink(
  cacheRoot: string,
  currentinsightAllRoots: string[],
  ...
  while (stack.length > 0 && examined < maxEntries) {
    ...
    for (const entry of entries) {
      ...
      if (!target || !looksLikeinsightAllPackagePath(target)) {
        continue;
      }

      const pointsAtCurrentinsightAll = currentinsightAllRoots.some((root) => isInside(root, target));
      if (!pointsAtCurrentinsightAll) {
