...
import { ensureInsightAllContext } from '../utils/openclaw-workspace';
...
export function createAgentsApi(_ctx: AgentsApiContext): CompleteHostServiceRegistry['agents'] {
  return {
    ...
    create: async (payload) => {
      ...
      void ensureInsightAllContext({ waitForAllConfiguredWorkspaces: true }).catch((err) => {
        console.warn('[agents] Failed to ensure InsightAll context after agent creation:', err);
      });
